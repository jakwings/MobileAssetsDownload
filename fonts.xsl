...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="$assets/dict">
      ...
      <xsl:variable name="asset.langs">
        <xsl:text>x&#10;</xsl:text>
        <xsl:for-each select="key[.='FontDesignLanguages'][1]/following-sibling::*[1][self::array]/string">
          <xsl:value-of select="concat(., '&#10;')" />
        </xsl:for-each>
      </xsl:variable>
      <xsl:variable name="asset.names">
        <xsl:for-each select="key[.='FontInfo4'][1]/following-sibling::*[1][self::array]/dict/key[.='FontFamilyName'][1]/following-sibling::*[1][self::string]">
      ...
      <!--
      <xsl:for-each select=".//key[.='FontInfo4']/following-sibling::*[1][self::array]/dict">
        <xsl:value-of select="normalize-space(key[.='FontFamilyName'][1]/following-sibling::*[1][self::string])" />
        <xsl:for-each select=".//key[.='DisplayNames']/following-sibling::*[1][self::dict]">
          <xsl:for-each select="key[.='en']/following-sibling::*[1][self::string]">
            <xsl:value-of select="concat('&#9;', normalize-space())" />
          </xsl:for-each>
          <xsl:for-each select="key[.!='en']/following-sibling::*[1][self::string]">
            <xsl:value-of select="concat('&#9;', normalize-space())" />
      ...
      <xsl:text>&#9;</xsl:text>
      <xsl:value-of select="concat(normalize-space($asset.langs), $asset.names)" />
      <xsl:text>&#10;</xsl:text>
    </xsl:for-each>
  </xsl:template>
